"""Regenerate Haskell-backed golden cases for the retained param-eq baseline."""
...
from egglog.exp.param_eq.paths import ARTIFACT_DIR, GOLDEN_PATH, REPO_ROOT, llvm_bin_dir, param_eq_data_dir
from egglog.exp.param_eq.pipeline import parse_expression, render_num
...
def main() -> None:
    ...
    portable_source = os.path.relpath(HASKELL_ROOT / "src" / "FixTree.hs", REPO_ROOT)
    portable_generator = os.path.relpath(Path(__file__).resolve(), REPO_ROOT)
    payload: dict[str, Any] = {
        "source_of_truth": portable_source,
