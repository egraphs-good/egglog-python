...
def test_all_scheduler_modes_handle_simple_expression(mode: str) -> None:
    expr = parse_expression("exp(log(abs(x0)))")
    report = run_paper_pipeline(expr, mode=mode)
...
def test_haskell_literal_mode_keeps_reduced_canaries_green(case_id: str) -> None:
    case = next(case for case in GOLDEN_CASES if case["case_id"] == case_id)
    report = run_paper_pipeline(parse_expression(case["source"]), mode="egglog-haskell-literal")
...
def test_reduced_pagie_second_pass_toy_reaches_haskell_form_in_both_modes() -> None:
    ...
    baseline = run_paper_pipeline(source, mode="egglog-baseline")
    literal = run_paper_pipeline(source, mode="egglog-haskell-literal")

    assert baseline.rendered == haskell_expected
    assert literal.rendered == haskell_expected

    egraph, _ = param_eq_hegg._run_single_pass_haskell_literal_egraph(source)
...
def test_golden_rewrite_tree_reaches_haskell_form(case: dict[str, object]) -> None:
    ...
    egraph, _ = param_eq_hegg._run_single_pass_haskell_literal_egraph(expr)
