...
polynomial({
    ...
    {Num(0.03139967317000205): 1, Num.var("x1"): 1}: 1.0,
    {
        ...
        polynomial({
            {Num(1.2522488356336676): 1}: 1.0,
            {exp(polynomial({{Num.var("x0"): 1, _expr_1: -1}: 1.0})): 1}: -1.0,
        }): 1,
    }: 1.0,
})
        ...
        polynomial({
            {Num.var("x0"): 2}: 1.0,
            {
                polynomial({
                    ...
                    {
                        Num.var("x1"): 1,
                        Num(0.04358235597907372): 1,
                        polynomial({{Num.var("x1"): 1}: 1.0, {Num(4.735723943783631): 1}: -1.0}): 1,
                    }: 1.0,
                                ...
                                abs(
                                    polynomial({
                                        ...
                                        {polynomial({{Num(-0.289): 1, Num(-0.327): -1}: 1.0}): 3}: -1.0,
                                        {
                                            ...
                                            polynomial({
                                                {Num(-0.045): 1}: 1.0,
                                                {polynomial({{Num.var("x0"): -2, Num(-0.464): 1}: 1.0}): 2}: -1.0,
                                            }): -1,
                                        }: -1.0,
                                    })
                                )
...
(
    ...
    * Num.var("x1")
    * (
        Num(1.1635766746115828)
        * Num.var("x0")
        * (
            Num.var("x0")
            - Num(1.096491354684671) * Num.var("x1")
            + Num(0.09649135468467125) * exp(Num.var("x1"))
            + Num(0.065716650770683)
        )
...
Num(-0.8529414239783971) * (
    ...
    * (
        Num.var("x0")
        * (
            Num(-2.824404573652556)
            + (Num.var("x0") + Num.var("x1") / Num(-0.9119998946891651) + exp(Num.var("x1")) / Num(10.363622764629516))
        )
        + Num.var("x1") / Num(-1.1635766746115828)
        + Num(0.4661137019136418)
    )
) / (exp(Num.var("x1")) - Num.var("x1") + exp(Num.var("x0") ** Num(2.0)) + Num(3.3628776435387486)) + Num(
    0.02765235981387666
)
# n_params=8
polynomial({
